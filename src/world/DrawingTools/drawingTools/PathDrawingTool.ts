...
export default class PathDrawingTool extends AbstractDrawingTool {
    ...
    update(point: DrawingPoint) {
        ...
        if (this.drawing) {
            this._toolMesh.scaling = BABYLON.Vector3.One().scaleInPlace(
                this.options.countPointRadius(point),
            );

            if (
                this.drawingPath.length === 0 ||
                this.drawingPath[this.drawingPath.length - 1].position
    ...
    public createDrawingMesh(): BABYLON.Mesh[] {
        ...
        const ribbonMesh = BABYLON.MeshBuilder.CreateTube(
            'tube' + Math.random(),
            {
                path: transformedPath.map(
                    (drawingPoint) => drawingPoint.position,
                ),
                //radius: .05,
                radiusFunction: (i, distance) =>
                    this.options.countPointRadius(transformedPath[i]),
            },
            this.world.scene,
        );
        ...
        const sphere1Mesh = BABYLON.MeshBuilder.CreateSphere(
            'sphere',
            { diameter: this.options.countPointRadius(transformedPath[0]) * 2 },
            this.world.scene,
        );
        ...
        const sphere2Mesh = BABYLON.MeshBuilder.CreateSphere(
            ...
            {
                diameter:
                    this.options.countPointRadius(transformedPath[last]) * 2,
            },
            this.world.scene,
        );
